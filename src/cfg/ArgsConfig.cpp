...
namespace ehb
{
    ...
    void argsConfig(WritableConfig& config, int argc, char* argv[])
    {
        ...
        { // parse all boolean values from the command line
            ...
            if (args.read("--profile", value)) config.setBool("profile", value);
            if (args.read("--debuglayer", value)) config.setBool("debuglayer", value);
            if (args.read("--apidumplayer", value)) config.setBool("apidumplayer", value);
        }
        {
        ...
        for (const auto& message : args.getErrorMessages())
            spdlog::get("log")->error("{}", message);
    }
} // namespace ehb
